const CACHE = "awakening-eden-v19";
const CORE = [
  "/", "/index.html", "/offline.html", "/eden-brand-v19.css", "/eden-v19.js",
...
self.addEventListener("activate", (event) => {
  event.waitUntil(caches.keys().then((keys) => Promise.all(keys.filter((key) => key.startsWith("awakening-eden-") && key !== CACHE).map((key) => caches.delete(key)))).then(() => self.clients.claim()));
});
